...
/*   getTimeZone1() {
  ...
  List<String> getDatePrayerTimes(int year, int month, int day,
  ...
  // return prayer times for a given date
  List<String> getPrayerTimes(DateTime date, double latitude,
  ...
  String floatToTime12(double time, bool noSuffix) {
    ...
    if (noSuffix == false) {
      if ((hours >= 0 && hours <= 9) && (minutes >= 0 && minutes <= 9)) {
        ...
        result = "0" + hours.toString() + ":" + (minutes).round().toString() + " " + suffix;
      } else if ((minutes >= 0 && minutes <= 9)) {
        result = hours.toString() + ":0" + (minutes).round().toString() + " " + suffix;
      } else {
        result = hours.toString() + ":" + (minutes).round().toString() + " " + suffix;
      }
  ...
  List<double> computeTimes(List<double> times) {
    ...
    double Fajr = this.computeTime(180 - _methodParams[this.getCalcMethod()][0], t[0]);
    ...
    double Maghrib = this.computeTime(_methodParams[this.getCalcMethod()][2], t[5]);
    double Isha = this.computeTime(
    _methodParams[this.getCalcMethod()][4], t[6]);
  ...
  List<double> adjustHighLatTimes(List<double> times) {
    ...
    // Adjust Fajr
    double FajrDiff = nightPortion(_methodParams[this.getCalcMethod()][0]) * nightTime;
    ...
    // Adjust Isha
    double IshaAngle = (_methodParams[this.getCalcMethod()][3] == 0) ? _methodParams[this.getCalcMethod()][4] : 18;
    double IshaDiff = this.nightPortion(IshaAngle) * nightTime;

    if (times[6] == double.nan || this.timeDiff(times[4], times[6]) > IshaDiff) {
      times[6] = times[4] + IshaDiff;
    }

    // Adjust Maghrib
    double MaghribAngle = (_methodParams[this.getCalcMethod()][1] == 0) ? _methodParams[(this.getCalcMethod())][2] : 4;
    double MaghribDiff = nightPortion(MaghribAngle) * nightTime;

    if (times[5] == double.nan || this.timeDiff(times[4], times[5]) > MaghribDiff) {
  ...
  double nightPortion(double angle) {
    ...
    if (_adjustHighLats == _angleBased)
      ...
      calc = 0.5;
    else if (_adjustHighLats == _oneSeventh)
      calc = 0.14286;
